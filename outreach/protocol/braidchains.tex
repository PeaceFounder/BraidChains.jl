...
%\item $signbraid(braid, ID, kc)$ where $kc \in KeyChain$. Retuns a one time signature of the braid with pseudonym identifier $ID$ whoose key is stored locally in the keychain. For simplicity we shall assume that it is equivalient to $sign("BRAID"||braid, K)$  where $id(K) = ID$.
\item 
